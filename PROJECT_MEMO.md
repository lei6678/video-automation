...
## 2026-08-08 多线并发 + 自动流水线方案设计

### 架构分析

全面梳理了当前后端并发瓶颈：
- **生图 Phase 1**：✅ 异步（httpx），单任务内 Semaphore(4)，任务间无限制
- **生图 Phase 2**：❌ PIL 落盘同步阻塞事件循环
- **视频合成**：❌ FFmpeg subprocess 阻塞，但用户已用 skip_ffmpeg，不是瓶颈
- **SQLite**：❌ 无 WAL 模式，写锁全库，多任务同时写会冲突
- **无全局并发控制**：无队列、无任务调度

### 当前 FFmpeg 实际用途

用户主要走剪映草稿导出（skip_ffmpeg），FFmpeg 仅用于：
1. TTS 分段合并（1-2 秒）
2. BGM 混音（2-3 秒）
→ 都不是瓶颈
...
| # | 改动 | 代码量 | 效果 |
|---|------|--------|------|
| 1 | SQLite WAL 模式 | 2 行 | 读写不互锁 |
| 2 | 全局 Semaphore（生图 ≤1 任务） | ~20 行 | 防 Fal.ai 限流 |
| 3 | 自动流水线 API | ~50 行 | 配音→生图→导出全自动 |
| **合计** | | **~1 小时** | |

操作方式：一个浏览器标签页 = 一个任务，各步骤独立并发，生图自动排队。

### 飞书集成方案

飞书多维表格作为任务面板：
- 字段：文案内容 / 视频标题 / 生图风格 / 状态 / 剪映草稿名 / 备注
- 后端定时扫描"待处理"行 → 自动流水线 → 回写状态 + 草稿名
- 用户只需粘贴文案 + 剪映最终确认

### 用户决策

先手动跑通几条验证质量，满意后再上自动化。
