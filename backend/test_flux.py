"""Flux-dev 测试：前 5 段 8:9 bench card 配图，对比 v7 (gpt-image-2)"""
import asyncio, os, sys, base64
sys.path.insert(0, '.')
from services.image_service import (
    plan_visual_arc, build_single_segment_prompt, _generate_fal_flux,
)
...
# Flux 8:9 请求尺寸（8 的倍数，略大于目标以支持超采样）
REQ_W, REQ_H = 1080, 1216   # 1080=135×8, 1216=152×8
TGT_W, TGT_H = 1080, 1214   # bench card 精确目标


async def main():
    # Step 1: LLM 视觉规划（复用 v7 框架）
    ...
    protagonist = vp.get('protagonist', '')
    print(f'Protagonist: {protagonist[:120]}...')
    ...
    for i in range(5):
        ...
        print(f'\n=== seg_{i:03d}_flux ({len(prompt)} chars) ===')
        print(prompt[:400])
        print()

        b64 = await _generate_fal_flux(prompt, width=REQ_W, height=REQ_H)
        if b64:
            ...
            print(f"Flux 原始尺寸: {actual_w}x{actual_h}")
            ...
            path = os.path.join(OUT, f'seg_{i:03d}_flux.png')
            pil_img.save(path, "PNG", optimize=True)
            pil_img.close()
            print(f'OK: seg_{i:03d}_flux.png ({os.path.getsize(path)} bytes)')
        else:
            print('FAILED')
