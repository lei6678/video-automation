...
排版规则:
- 第一行是铺垫短行（白色展示），交代对象或背景
- 第二行是点题长行（金色展示），给出核心观点/悬念/情绪钩子，比第一行更有分量
- 两行连读语义通顺，有递进或转折的节奏感
- **优先在逗号、分号处断句**——逗号本身就是最自然的语义暂停点，第一行停在逗号结尾，第二行接后面的内容
- 不加书名号以外的标点，不用英文
- **绝对禁止：不允许增删改任何一个字，只能从原文中选取完整词语断行拆分。** 原标题的所有字符必须全部保留。如果标题很短无法拆分，把全部内容放在第二行。
